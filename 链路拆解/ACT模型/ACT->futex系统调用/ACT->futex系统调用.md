...
```c
    ...
    lock()
    {
      // 这是 PyTorch (或任何 C++ 代码) 调用 std::mutex::lock() 时真正执行的代码。
      // 它只是薄薄地包装了一层，直接把内部的 _M_mutex 传给了底层 C 接口。
      // 在 Linux 下，__gthread_mutex_lock 实际上是宏替换成了 glibc 的 pthread_mutex_lock(&_M_mutex) 函数。
      // 也就是说，到了这一句，C++ 就把工作彻底移交给了 Glibc。如果发生锁争用，则 Glibc 会去调 futex。
