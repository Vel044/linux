...
Python 层调用的 `self.videocapture.read()` 实际上绑定到了 OpenCV 内部的 C++ 类 `cv::VideoCapture::read()`.
...
bool CvCaptureCAM_V4L::tryIoctl(unsigned long ioctlCode, void *parameter, bool failIfBusy, int attempts) const
{
    ...
    while (true)
    {
        ...
        struct timeval tv;
        tv.tv_sec = param_v4l_select_timeout;  // 默认 10 秒超时
        tv.tv_usec = 0;
...
bool CvCaptureCAM_V4L::grabFrame()
{
    ...
    if (FirstCapture)
    {
        ...
        for (__u32 index = 0; index < req.count; ++index) {
            ...
            if (!tryIoctl(VIDIOC_QBUF, &buf)) {  // 将缓冲区放回队列
                return false;
            }
        }
...
摄像头是**慢速硬件**，数据从 USB 线传输到内存需要时间（毫秒级）。工作流程：

```
1. 应用程序调用 grabFrame() → read_frame_v4l2()
2. tryIoctl(VIDIOC_DQBUF) 尝试从队列取帧
3. 如果队列为空（EAGAIN），进入 select 等待
   ├── select(设备描述符, 超时10秒)
   ├── 线程进入睡眠，不消耗 CPU
   └── 等待 USB 摄像头新数据到达

4. USB 硬件产生中断，数据拷贝到 V4L2 缓冲区
5. 内核唤醒等待队列中的线程
6. select 返回，tryIoctl 成功取到帧数据
7. grabFrame() 返回，OpenCV 拿到数据
```
