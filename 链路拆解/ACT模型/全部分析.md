...
```mermaid
flowchart
    A[机器人 Robot] --> B["robot.get_observation()<br/>得到硬件观测数据：<br/>- 两个手臂（每个手臂6个关节）<br/>- 两个摄像头"]
    B --> C["robot_observation_processor<br/>机器人观测后处理"]
    C --> D["processed_obs<br/>处理后的观测"]

    D --> E{"动作逻辑 ACTION LOGIC"}
    E --> F["来自遥操作器（Teleoperator）"]
    E --> G["来自策略（Policy）"]

    F --> H["teleop.get_action<br/>raw_action（原始动作）"]
    H --> I["teleop_action_processor<br/>遥操作动作后处理"]
    I --> J["processed_teleop_action<br/>处理后的遥操作动作"]

    G --> K["predict_action<br/>策略推理动作"]
    K --> L["processed_policy_action<br/>处理后的策略动作"]

    J --> M["robot_action_processor<br/>机器人动作发送前处理"]
    L --> M
    M --> N["robot_action_to_send<br/>待发送动作"]
    N --> O["robot.send_action(action)<br/>发送到手臂执行：<br/>- 双臂推理：两个手臂的数据<br/>- 单臂推理：一个手臂的数据"]
    O --> P["机器人执行动作"]
    P --> Q["保存到数据集 Save to Dataset"]
    Q --> R["记录日志 / 等待下一轮循环"]
```
...
```mermaid
sequenceDiagram 
    participant T as 相机读取线程(_read_loop)
    participant OS as OS/驱动唤醒机制
    participant M as 主线程(record_loop)
    participant BUF as 内存缓存(latest_frame + event)

    loop 持续后台读取
        T->>OS: 发起 read() 读取相机
        Note over T,OS: 摄像头持续产出约30 FPS（约每33ms一帧）
        alt 当前没有新帧
            OS-->>T: 线程休眠，等待设备就绪再唤醒
        else 读到新帧
            OS-->>T: 返回图像帧
            T->>BUF: 写入 latest_frame
            T->>BUF: new_frame_event.set()
        end
    end

    loop 每个控制周期
        M->>M: 执行 观察→推理→动作
        M->>BUF: 需要新观测时调用 async_read()
        alt event 已就绪
            BUF-->>M: 直接返回 latest_frame（加锁读取）
        else event 未就绪
            M->>BUF: 最多等待 timeout_ms(默认200ms)
            alt 等到新帧
                BUF-->>M: 返回 latest_frame
            else 超时
                BUF-->>M: TimeoutError
            end
        end
